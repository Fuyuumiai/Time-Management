<template>
	<view class="wrap">
		<u-navbar :background="{ backgroundColor: '#ffffff' }"  :border-bottom="false" title="目标列表">
		</u-navbar>
		<u-gap height="12" bg-color="#fafafa"></u-gap>
		<scroll-view scroll-y style="height: 100%;width: 100%;" @scrolltolower="queryTarget()">
			<view class="target" v-if="targetArr.length">
			<view class="target-box" v-for="(item, i) in targetArr" :key="i"  @click="popupSelect(i)">
		...
		<u-loadmore v-if="targetArr.length" :status="pageQuery.status" margin-top="30" :loadText="loadText" />
		</scroll-view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			popupSelect(i){
				if (this.status === 1) {
					if (this.targetArr[i].numType !== 1) {
						return
					}
				}
				this.onSelect = this.targetArr[i]
			...
			getTargetById(targetId, type = 0) {
				this.$apis.getTargetById(targetId).then(({data}) => {
					if (data.code === 200) {
						let i = this.fundById(this.targetArr, planId)
						if (i !== -1) {
							this.$set(this.targetArr, i, data.data)
						} else {
							this.targetArr.unshift(data.data)
						}
					} else {
			...
			hadleNumber(){
				...
				this.$apis.upDataTarget({
					...
					if (res.data.code === 200) {
						...
						if (target.finish) {
							this.targetArr.splice(this.onSelectIndex, 1)
						}
			...
			queryTarget(){
				if (!this.pageQuery.next) {
					return false
				}
				this.pageQuery.status = 'loading'
				if (!this.userId) { return this.$refs.uToast.show({title: '用户未登录'}) }
				this.$apis.queryTypeTarget({
					userId: this.userId,
					status: this.status,
					page: this.pageQuery.page,
					num: this.pageQuery.num,
				}).then(({data}) => {
					if (data.code === 200) {
						this.pageQuery.page = data.data.page
						this.pageQuery.next = data.data.next
						if (this.pageQuery.next) {
							this.pageQuery.status = 'loadmore'
						} else {
							this.pageQuery.status = 'nomore'
						}
						this.targetArr.push(...data.data.list)
					} else {
						this.$refs.uToast.show({title: data.message})
					}
				}).catch(err => {
